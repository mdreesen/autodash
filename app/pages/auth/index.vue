...
<template>
  <div class="min-h-screen bg-[#F8F9FA] text-[#1A1D20] font-sans flex flex-col justify-between pb-8 px-4 antialiased">
    ...
    <div class="pt-16 text-center max-w-sm mx-auto w-full">
      <div class="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-white border border-zinc-200/80 mb-3 shadow-sm">
    ...
    <div class="max-w-sm w-full mx-auto bg-white border border-zinc-200/60 rounded-3xl p-6 shadow-md my-auto">
      ...
      <div class="bg-zinc-100/80 p-1 rounded-xl border border-zinc-200/40 flex items-center mb-6 relative overflow-hidden">
        <button 
      ...
      <div v-if="errorMessage" class="mb-5 p-3.5 bg-red-500/5 border border-red-500/10 rounded-xl text-xs text-red-500 font-semibold leading-tight flex items-center gap-2 animate-fadeIn">
      ...
      <form @submit.prevent="handleAuthAction" class="space-y-4">
        ...
        <div v-if="!isLoginView" class="space-y-4 class-animate-fadeIn">
          <div>
            ...
            <input v-model="authForm.name" type="text" placeholder="Alex Mercer" class="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:border-[#FF5A00] focus:bg-white text-zinc-900 placeholder:text-zinc-400 transition-all" />
          </div>
        ...
        <div>
          ...
          <input v-model="authForm.email" type="email" autocomplete="email" placeholder="name@company.com" class="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:border-[#FF5A00] focus:bg-white text-zinc-900 placeholder:text-zinc-400 transition-all" />
        </div>

        <div>
          <label class="block text-[11px] font-bold text-zinc-600 mb-1">Password Shield</label>
          <input v-model="authForm.password" type="password" autocomplete="current-password" placeholder="••••••••••••" class="w-full bg-zinc-50 border border-zinc-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:border-[#FF5A00] focus:bg-white text-zinc-900 placeholder:text-zinc-400 transition-all font-mono text-xs" />
        </div>

        <!-- DoorDash Style Accent Submit Trigger -->
        <button :disabled="isLoading" type="submit" class="w-full bg-[#FF5A00] text-white font-extrabold text-xs uppercase tracking-wider py-4 rounded-xl shadow-md shadow-[#FF5A00]/10 hover:bg-[#e04f00] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-6">
